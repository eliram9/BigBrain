            ...
            {podcasts.map((podcast, index) => (
                <li key={index} className="mb-2 flex items-top bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                    <div className="mr-4 mb-2">
                        <img src={podcast.image} alt={podcast.title} className="w-20 object-cover rounded-lg" />
                    </div>
                    <div>
                        <a href={podcast.link} target="_blank" rel="noopener noreferrer">
                            <h4 className="font-semibold hover:underline">{podcast.title}</h4>
                        </a>
                        <p className='mt-1 text-md'>host: {podcast.host}</p>
                    </div>
                </li>
            ))}
        </ul>
